...
use std::fs::FileType;
use std::os::unix::fs::MetadataExt; // todo! windows-compatible
...
impl FileStat {
    pub fn new(
        ...
        match md {
            Ok(md) => {
                let block_size = file_real_size_fast(&path, &md).ok();
    ...
    pub fn read_dir(&self, deref: bool, tag: Span) -> Result<impl Iterator<Item = Result<FileStat, ShellError>>, ShellError>
    {

        match self.path.read_dir() {
            Ok(read_dir) => {
                Ok(read_dir.map(move |item|{
                    match item {
                        Ok(dir_entry) => {
                            FileStat::new(dir_entry.path(), deref, tag)
                        },
                        Err(err) => Err(IoError::new(err, tag, self.path.clone()).into())
                    }
                }))
            },
            Err(err) => Err(IoError::new(err, tag, self.path.clone()).into())
